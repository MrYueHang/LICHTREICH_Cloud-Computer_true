...
- ✅ REIHE DEPLOYED 18.07 (VPS, Muster `/opt/andock.sh SUB PORT`): Stirling-PDF→pdf · docassemble→formulare · Twenty→crm · Metabase→metabase. j-lawyer = Desktop-Client, KEIN Browser-Modul → nur Klau-Quelle. OpenProject→projekte (Hetzner+Swap) · copyparty→dateien (Netzwerkplatte).
