...
export default function Home() {
  ...
  return (
    <main className="portfolio-shell">
      ...
      <section id="main-content" className="hero-section" aria-labelledby="hero-title"><div className="hero-copy"><motion.p className="eyebrow" {...motionProps}><CircleDot size={13} /> immersive frontend / human-scale systems</motion.p><motion.h1 id="hero-title" className="hero-title" {...motionProps} transition={reducedMotion ? undefined : { ...reveal.transition, delay: 0.08 }}>Interfaces<br /><em>with a pulse.</em></motion.h1><motion.p className="hero-lede" {...motionProps} transition={reducedMotion ? undefined : { ...reveal.transition, delay: 0.16 }}>{portfolioContent.profile.intro}</motion.p><motion.div className="hero-actions" {...motionProps} transition={reducedMotion ? undefined : { ...reveal.transition, delay: 0.24 }}><button className="button button-primary" type="button" onClick={() => scrollToId('work')}>Arbeiten erkunden <ArrowDownRight size={17} /></button><a className="text-link" href="#contact">{portfolioContent.profile.availability} <ArrowUpRight size={16} /></a></motion.div><div className="hero-footnote"><span>01 / 04</span><span>Designing the space between human intent and machine complexity.</span></div></div><WebGLHero reducedMotion={reducedMotion} lowPower={lowPower} /></section>

      <section className="signal-strip" aria-label="Portfolio-Fokus"><div className="signal-label">Selected signals</div><div className="signal-list"><span>WebGL</span><span>Shader systems</span><span>Motion systems</span><span>Content architecture</span><span>Frontend craft</span></div></section>
